...
export default function Xwhysi4() {
  ...
  const [polyrhythmActive, setPolyrhythmActive] = useState(false);
  ...
  useEffect(() => {
    ...
    const invertInterval = beatMs;
    const blurInterval = beatMs * 1.5;

    const invertTimer = setInterval(() => {
      setInvert((prev) => !prev);
    }, invertInterval);
    ...
    >
      ...
      <div className="fixed inset-0 overflow-hidden">
        <video
          ...
          style={{ opacity: 0.6, transform, filter }}
        >
        ...
        <section className="min-h-screen flex flex-col items-center justify-center px-6 py-20">
          <div className="w-full max-w-5xl">
            ...
            <div className="flex justify-center gap-4 mb-8">
              <button
                onClick={() => setPolyrhythmActive(!polyrhythmActive)}
                className={`px-6 py-3 rounded-full text-sm font-medium transition-all ${
                  polyrhythmActive ? "bg-violet-600" : "bg-zinc-800"
                }`}
              >
                {polyrhythmActive ? "Polyrhythm ON" : "Polyrhythm OFF"}
              </button>
              <div className="flex items-center gap-2 text-zinc-400 text-xs">
                <span className={invert ? "text-violet-400" : ""}>Invert</span>
                <span>|</span>
                <span className={blur ? "text-pink-400" : ""}>Blur</span>
                <span className="ml-2">120 BPM • 3:2</span>
              </div>
            </div>

            <p className="text-center text-xs opacity-40 text-white">
              ← → zoom presets • ↑ ↓ speed • H hide UI
            </p>
          </div>
        </section>
      </div>
